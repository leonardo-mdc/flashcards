(function () {
    ...
    function renderEditFields(patternType, contentData) {
        ...
        if (patternType === 'multiple_choice') {
            ...
            html = `
                ...
                <div>
                    <label class="block font-bold mb-1">Explanation (optional):</label>
                    <textarea id="editExplanation" class="form-textarea" rows="3" placeholder="Explain why this is correct...">${escapeHtml(contentData.explanation || '')}</textarea>
                </div>
            `;
    ...
    function setupUploadHandlers(patternType) {
        const btnId = patternType === 'image_description' || patternType === 'image_mcq' ? 'editImageUploadBtn' : patternType === 'audio_listening' ? 'editAudioUploadBtn' : null;
        ...
        if (patternType === 'multiple_choice') {
            ...
            frontHtml = `
                <div class="flex flex-col md:flex-row gap-2 w-full min-h-[200px]">
                    <div class="flex items-center justify-center md:w-1/2 bg-gray-50 rounded-xl p-2">
                        ${hasImg ? `<img src="${escapeHtml(imgUrl)}" alt="${escapeHtml(title)}" class="max-h-32 md:max-h-full w-full object-contain rounded-lg" onerror="this.style.display='none'">` : `<div class="text-4xl text-gray-300">🖼️</div>`}
                    </div>
                    <div class="flex flex-col justify-center md:w-1/2 gap-1">
                        ${options.map((opt, idx) => `<div class="quiz-option-preview text-xs py-1.5 px-2">${String.fromCharCode(65+idx)}. ${formatBreaks(escapeHtml(opt))}</div>`).join('')}
            ...
            frontHtml = `
                <div class="flex flex-col items-center justify-center min-h-[200px]">
                    ...
                    <div class="text-center font-bold text-xl mb-2">${formatBreaks(escapeHtml(title))}</div>
            ...
            frontHtml = `
                <div class="flex flex-col items-center justify-center min-h-[200px]">
                    <div class="text-4xl text-center font-bold">${formatBreaks(escapeHtml(title))}</div>
        ...
        if (patternType === 'multiple_choice') {
            ...
            backHtml = `
                <div class="text-center">
                    ...
                    <p class="text-sm text-gray-600">${formatBreaks(escapeHtml(contentData.explanation || ''))}</p>
                </div>
            `;
        } else if (patternType === 'gap_fill') {
            ...
            backHtml = `
                <div class="text-center">
                    <h3 class="text-2xl text-blue-700 marker-underline mb-3">${formatBreaks(escapeHtml(title))}</h3>
            ...
            backHtml = `
                <div class="text-center">
                    <h3 class="text-xl text-green-700 marker-underline mb-3">${formatBreaks(escapeHtml(title))}</h3>
    ...
    async function saveCard() {
        ...
        const cardData = {
            ...
            question_text: patternType === 'multiple_choice' ? contentData.question_text : patternType === 'image_mcq' ? (contentData.question_text || '') : '',
        ...
        if (pattern === 'multiple_choice') {
            ...
            return `
                <div class="text-center w-full">
                    <h3 class="text-base text-green-700 marker-underline mb-2">✓ Answer</h3>
                    <div class="bg-green-50 p-2 rounded-xl border-2 border-green-300 mb-2">
                        <p class="text-sm font-bold">${String.fromCharCode(65+correctIdx)}. ${formatBreaks(escapeHtml(options[correctIdx]))}</p>
                    </div>
                    ${selectedIdx !== undefined && selectedIdx !== null ? `
                    <div class="text-sm mb-1 ${selectedIdx === correctIdx ? 'text-green-600' : 'text-red-600'}">
                        ${selectedIdx === correctIdx ? '✅ Correct!' : `❌ You answered: ${String.fromCharCode(65+selectedIdx)}. ${formatBreaks(escapeHtml(options[selectedIdx]))}`}
                    </div>` : '<p class="text-xs text-gray-500 mb-2">You did not select an answer.</p>'}
                    ${explanation ? `<div class="bg-blue-50 p-2 rounded-xl border border-blue-200 mt-2"><p class="text-sm text-gray-800">💡 ${explanation}</p></div>` : ''}
                </div>
            `;
        } else if (pattern === 'gap_fill') {
    ...
    function openTestCardModal() {
        ...
        // Bind MCQ options
        if (patternType === 'multiple_choice' || patternType === 'image_mcq') {
            setTimeout(() => {
