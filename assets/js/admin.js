(function () {
    ...
    function renderPreview(frontEl, backEl, type, title, contentData) {
        ...
        if (type === 'image_mcq') {
            ...
            front = `<div class="text-center"><div class="text-lg font-bold mb-2">${t}</div><div class="flex flex-col md:flex-row gap-3 min-h-[200px]"><div class="flex items-center justify-center md:w-1/2 bg-gray-50 rounded-xl p-2">${hasImg ? `<img src="${esc(cd.image_url)}" class="max-h-32 object-contain">` : '<div class="text-5xl text-gray-300">🖼️</div>'}</div><div class="flex flex-col justify-center md:w-1/2 gap-2"><p class="text-sm font-bold text-center md:text-left">Select the correct answer:</p>${opts.map((o,i) => `<div class="quiz-option-preview text-sm py-1">${String.fromCharCode(65+i)}. ${fmtBreaks(esc(o))}</div>`).join('')}</div></div></div>`;
